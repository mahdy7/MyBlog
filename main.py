...
def register():
    ...
    if request.method == 'POST':
        if not User.query.filter_by(email=request.form.get('email')).first():
            ...
            flash('This email already exists, login not register idiot!')
...
def show_post(post_id):
    ...
    if comment.validate_on_submit():
        if current_user.is_authenticated:
            ...
            return redirect(url_for('login'))
    return render_template("post.html", post=requested_post, logged_in=current_user.is_authenticated, form=comment)
